...
import type { FilterProcessor } from './openseadragon-filtering';
...
class WebGLFilterRenderer {
  ...
  applyFilters(filters: WebGLFilterConfig[]) {
    ...
    this.viewer.canvas.style.opacity = '0';
    const viewerCanvas = this.viewer.drawer.canvas;
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, viewerCanvas);
    ...
    for (let i = 0; i < filters.length; i++) {
      ...
      if (!isLastFilter) {
        ...
        tempTextures.push(outputTexture);
...
export const initWebGLFiltering = (viewer: any) => {
  const renderer = new WebGLFilterRenderer(viewer);
